...
const DepartmentManagement = () => {
  const { isLoggedIn, organisation_id: organisation_id } = useContext(LoginContext);
  ...
  const fetchData = async () => {
      try {
        const response = await fetch(
          `http://194.195.117.38:8000/org-get-department`
        );
        if (response.ok) {
  ...
  const handleDeptSubmit = async () => {
    ...
    const existingDept = deptData.find(dept => department_name === department_name);
    if (existingDept) {
    ...
    const response = await aiDishaApi.post("/org-insert-department",
       {
...
const handleDeptEdit = async (dept_id) => {
  ...
  try {
    ...
    const existingDept = deptData.find(dept => dept.department_name === department_name && dept.dept_id !== dept_id);
    if (existingDept) {
      console.error("Department name already exists");
      Toast.error("Department name already exists");
      return;
    ...
    const updateResponse = await aiDishaApi.put(`/admin-departments-update/${organisation_id}/${dept_id}`, {
      department_name: department_name,
    ...
    const updatedDeptData =deptData.map(dept => {
      if (dept.dept_id === deptId) {
        return { ...dept, department_name: department_name };
      } else {
        return dept;
      }
    });
...
try {
  ...
  const response = await aiDishaApi.patch(`/admin-departments-status/${organisation_id}/${deactivateId}/status`, {
    status: "Inactive",
  });
