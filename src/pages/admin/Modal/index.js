...
const Modal = ({ isOpen, onClose, title,action }) => {
  ...
  const openSureModal = () => {
    console.log("In open sure modal func")
    onClose(); // Close the current modal
    setSureModalOpen(true); // Open the SureModal
  };

  const handleConfirm = () => {
    console.log("In handle confirm func")
    action();
  ...
  return (
    <>
      ...
      <SureModal
        ...
        onConfirm={handleConfirm} // Pass handleConfirm function as prop to SureModal
        title={title}
      />
    </>
  );
